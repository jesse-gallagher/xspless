...
public class ServletFactory implements IServletFactory {
	...
	public ServletMatch getServletMatch(final String contextPath, final String path) throws ServletException {
		...
		if(pieces.length >= 3) {
			...
			String servletNameProperCase = servletName.substring(0, 1).toUpperCase() + servletName.substring(1);

			try {
				Class<?> clazz = Class.forName("servlet." + servletNameProperCase);
				return new ServletMatch(module_.createServlet(clazz.getName(), servletNameProperCase, null), "", path);
